...
function main(sources) {
  ...
  const workerB$ = sources.WWB.map(m => {
    mMres.ret(m.data).bnd(() => mM35.ret('And that is how I like to handle asynchronous code.'));
  });
  ...
  var primeFib$ = fibKeyPress5$.map(e => {
    workerB.postMessage(e.target.value).bnd(() => mM35
      .ret('Asynchronous addendum: the largest computed prime Fibonacci number is', mMres.x[2][nNres.x.length - 1])
    );
  });
