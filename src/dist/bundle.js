...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	...
	var e2 = (0, _dom.h)('pre.turk', '  var c = m.ret(0).bnd(add,3).bnd(cube).bnd(log, "The values m\'s and c\'s \n  x attributes are " + get(m) + " and " + get(c) + " respectively.",  "m"  )   ');

	var e2x = (0, _dom.h)('pre', '   Output: From m: The values of m\'s and c\'s x attributes are 0 and 27 respectively.  ');
	...
	var e4x = (0, _dom.h)('pre', '  Output: From m: The values of m\'s and c\'s x attributes are 27 and 27 respectively.  ');
	...
	var e6x = (0, _dom.h)('pre', '  Output: From m: get(m) and get(m2) and get(m3) are  0, 3 and 27 respectively.  ');
	...
	exports.default = { MonadMaybe: MonadMaybe, fmapA: fmapA, monad: monad, equals: equals, fmap: fmap, opM: opM, e1: e1, e2: e2, e2x: e2x, e3: e3, e4: e4, e4x: e4x, e6: e6, e6x: e6x, fib: fib, driver: driver, messages: messages, next: next, monadIt: monadIt, MonadSet: MonadSet, updateCalc: updateCalc, arrayFuncs: arrayFuncs, travel: travel, nums: nums, cleanup: cleanup, ret: ret, C42: C42, newTask: newTask, process: process, mM$task: mM$task, addString: addString, colorClick: colorClick, edit: edit, testZ: testZ, quad: quad, runTest: runTest, todoStream: todoStream, inc: inc, seed: seed, add: add, traverse: traverse, MonadState: MonadState, primesMonad: primesMonad, fibsMonad: fibsMonad, primeFibInterface: primeFibInterface, tr3: tr3, fpTransformer: fpTransformer, innerHTML: innerHTML, factorsMonad: factorsMonad, factorsInput: factorsInput, playerMonad: playerMonad, promise: promise, promiseSnippet: promiseSnippet, timeout: timeout, timeoutSnippet: timeoutSnippet, examples: examples, examples2: examples2, async: async };
...
/***/ function(module, exports, __webpack_require__) {
	...
	function updateElms(style, nums, sc, gl) {
	    elms = [(0, _dom.h)('button#0.num', { style: { display: style[0] } }, nums[0]), (0, _dom.h)('button#1.num', { style: { display: style[1] } }, nums[1]), (0, _dom.h)('button#2.num', { style: { display: style[2] } }, nums[2]), (0, _dom.h)('button#3.num', { style: { display: style[3] } }, nums[3])];
	...
	function main(sources) {
	    ...
	    var messages$ = sources.WS.map(function (e) {
	        mMtem.ret(e.data.split(',')).bnd(function (v) {
	            ...
	            console.log('<><><><><><><><><><><><><><><><>  INCOMING  <><><><><><><> >>> In messages. e amd v are ', e, v);
	            mMZ10.bnd(function () {
	                return mMnums.ret([v[3], v[4], v[5], v[6]]).bnd(newR);
	            });
	            mMZ11.bnd(function () {
	                return socket.send('CG#$42,' + get(pMgroup) + ',' + get(pMname) + ',' + get(pMscore) + ',' + getpM(goals) + 'l');
	            });
	            mMZ12.bnd(function () {
	            ...
	            mMZ18.bnd(function () {
	                if (group != 'solo' || name == v[2]) {
	                    updatePlayers(e.data);
	                }
	            });
	        });
	        mMtemp.ret(e.data.split(',')[0]).bnd(next, 'CA#$42', mMZ10).bnd(next, 'XX#$42', mMZ11).bnd(next, 'CD#$42', mMZ13).bnd(next, 'CE#$42', mMZ14).bnd(next, 'EE#$42', mMZ15).bnd(next, 'DD#$42', mMZ17).bnd(next, 'NN#$42', mMZ18);
	    });
	    ...
	    function updateCalc(ar, op) {
	        ...
	        if (result == 20) {
	            pMscore.bnd(add, 1).bnd(tscore).bnd(pMscore.ret).bnd(function (a) {
	                return score2(a);
	            });
	            return;
	        } else if (result == 18) {
	            pMscore.bnd(add, 3).bnd(tscore).bnd(pMscore.ret).bnd(function (a) {
	                return score2(a);
	            });
	            return;
	        } else {
	            mMnums.bnd(push, result).bnd(mMnums.ret).bnd(function (x) {
	                return updateElms(test2(get(mMstyle), x), x, get(pMscore), get(pMgoals));
	            });
	            mM8.ret(0);
	            mM3.ret([]);
	        }
	    ...
	    function score2(scor) {
	        ...
	        if (scor != 25) {
	            mMnums.bnd(push, scor).bnd(mMnums.ret).bnd(function (ar) {
	            ...
	            playerMonad.run(get(pMscore), get(pMgoals.bnd(add, 1).bnd(pMgoals.ret), elms)).bnd(function (s) {
	    ...
	    var timeoutAction$ = timeoutClicks$.map(function () {
	        document.getElementById('timeout2').innerHTML = '';
	        document.getElementById('timeout3').innerHTML = '';
	        m.ret(3).bnd(m.ret).bnd(display, 'timeout2', 'get(m) is ' + ' ' + get(m)).bnd(m.ret).bnd(timeout2, 1.5, m, [function () {
	            return m.bnd(cube).bnd(m.ret).bnd(display, 'timeout2', 'get(m) is ' + ' ' + get(m)).bnd(m.ret).bnd(timeout2, 2, m, [function () {
	                return m.bnd(add, 15).bnd(m.ret).bnd(display, 'timeout2', 'get(m) is ' + ' ' + get(m)).bnd(m.ret).bnd(display, 'timeout3', 'The meaning of everything was computed to be ' + get(m) + '.');
	            }]);
	        }]);
	    });
	    ...
	    var rollClickAction$ = rollClick$.map(function () {
	        socket.send('CA#$42,' + get(pMgroup) + ',' + get(pMname) + ',6,6,12,20');
	        pMscore.bnd(add, -1).bnd(pMscore.ret).bnd(function (v) {
	            return socket.send('CG#$42,' + get(pMgroup) + ',' + get(pMname) + ',' + v + ',' + get(pMgoals));
	        });
	    });
