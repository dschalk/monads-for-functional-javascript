	...
	function main(sources) {
	    ...
	    var timeoutAction$ = timeoutClicks$.map(function () {
	        ...
	        m.ret(3, 'm').bnd(timeout2, 1, m, [function () {
	            return m.bnd(cube, m).bnd(display, 'timeout2', 'get(m) is ' + ' ' + get(m), m).bnd(timeout2, 2, m, [function () {
	                return m.bnd(add, 15, m).bnd(display, 'timeout2', 'get(m) is ' + ' ' + get(m), m).bnd(display, 'timeout3', 'The meaning of everything was computed to be' + ' ' + get(m), m);
	            }]);
	        }]);
	    });
