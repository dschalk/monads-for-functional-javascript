...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	...
	exports.default = { fmapA: fmapA, monad: monad, equals: equals, fmap: fmap, opM: opM, e1: e1, e2: e2, e2x: e2x, e3: e3, e4: e4, e4x: e4x, e6: e6, e6x: e6x, driver: driver, messages: messages, monadIt: monadIt, MonadSet: MonadSet, updateCalc: updateCalc, arrayFuncs: arrayFuncs, travel: travel, nums: nums, cleanup: cleanup, ret: ret, C42: C42, newTask: newTask, process: process, mM$task: mM$task, addString: addString, colorClick: colorClick, edit: edit, testZ: testZ, quad: quad, runTest: runTest, todoStream: todoStream, inc: inc, seed: seed, add: add, traverse: traverse, MonadState: MonadState, primesMonad: primesMonad, fibsMonad: fibsMonad, primeFibInterface: primeFibInterface, tr3: tr3, fpTransformer: fpTransformer, innerHTML: innerHTML, factorsMonad: factorsMonad, factorsInput: factorsInput, playerMonad: playerMonad, promise: promise, promiseSnippet: promiseSnippet, timeout: timeout, timeoutSnippet: timeoutSnippet, examples: examples, examples2: examples2, async: async };
	...
	function main(sources) {
	    ...
	    var messages$ = sources.WS.map(function (e) {
	        ...
	        mMtem.ret(e.data.split(',')).bnd(function (v) {
	            console.log('<><><><><><><><><><><><><><><><>  INCOMING  <><><><><><><> >>> In messages. e amd v are ', e, v);
	            mMZ10.bnd(function () {
	                pMnums.ret([v[3], v[4], v[5], v[6]]).bnd(function (w) {
	                    return test3(w).bnd(pMstyle.ret).bnd(function (x) {
	                        return numsDisplay = displayNums(x, w);
	                    });
	                });
	                pMscore.ret(v[7]);
	                pMgoals.ret(v[8]);
	        ...
	        mMtemp.ret(e.data.split(',')[0]).bnd(cow, 'CA#$42', mMZ10).bnd(cow, 'XX#$42', mMZ11).bnd(cow, 'CD#$42', mMZ13).bnd(cow, 'CE#$42', mMZ14).bnd(cow, 'EE#$42', mMZ15).bnd(cow, 'DD#$42', mMZ17).bnd(cow, 'NN#$42', mMZ18);
	    });
	    ...
	    function displayNums(a, b) {
	        numsDisplay = [(0, _dom.h)('button#0.num', { style: { display: a[0] } }, b[0]), (0, _dom.h)('button#1.num', { style: { display: a[1] } }, b[1]), (0, _dom.h)('button#2.num', { style: { display: a[2] } }, b[2]), (0, _dom.h)('button#3.num', { style: { display: a[3] } }, b[3])];
	        return numsDisplay;
	    };
	    ...
	    var groupPressAction$ = groupPress$.map(function (e) {
	        if (e.keyCode == 13) {
	            ...
	            pMgroup.ret(gr).bnd(pMgroup.ret).bnd(function (v) {
	                return socket.send('CA#$42,' + v + ',' + get(pMname) + ',6,6,12,20,0,0');
	            });
	        }
	    });
	    ...
	    var numClickAction$ = numClick$.map(function (e) {
	        pMnums.bnd(spliceM, e.target.id, 1).bnd(pMnums.ret).bnd(function (x) {
	            return test3(x).bnd(pMstyle.ret).bnd(function (y) {
	                return numsDisplay = displayNums(y, x);
	            });
	        });
	    ...
	    function updateCalc(ar, op) {
	        ...
	        if (result == 20) {
	            ...
	            pMnums.bnd(push, result).bnd(pMnums.ret).bnd(function (x) {
	                test3(x).bnd(pMstyle.ret).bnd(function (y) {
	                    return numsDisplay = displayNums(y, x);
	                });
	            });
	            mM8.ret(0);
	    ...
	    var primeFib$ = fibKeyPress5$.map(function (e) {
	        ...
	        if (e.keyCode == 13) {
	            var res = fibsMonad.run([1, 2, e.target.value, [0, 1]]).bnd(function (fibsState) {
	    ...
	    function display(x, id, string) {
	        var mon = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : mMdisplay;
