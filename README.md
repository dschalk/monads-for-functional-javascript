...
In both instances of MonadState, the run() method takes an array of four elements. The following functions are abstractions over run that take one argumant and make sure that all four of the elements in the array presented to run() are correct. The argument provided to runFib determines the length of the array of Fibonacci numbers. The number provided to runState determins the upper bound on the array of Fibonacci numbers. To compute an array of prime Fibonacci numbers, we first generate an array of Fibonacci numbers named "fibs" using runFib. Then we create an array of Fibonacci numbers named "fibs2" bounded by the square root of the largest Fibonacci number in the array. It is sufficient to check divisibility by the array of prime numbers generated by "runPrime( fibs[fibs2.length] ).
