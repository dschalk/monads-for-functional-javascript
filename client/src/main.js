  ...
  const calcStream$ = merge( timeoutAction$, factorsAction$, forwardAction$, backAction$, dummyAction$, primeFib$, fibPressAction$, runTestAction$, quadAction$, testWAction$, testZAction$, testQAction$, edit1Action$, edit2Action$, colorAction$, deleteAction$, newTaskAction$, chatClickAction$, gameClickAction$, todoClickAction$, captionClickAction$, groupPressAction$, rollClickAction$, messagePressAction$, loginPressAction$, messages$, numClickAction$, opClickAction$ );
    ...
    ret(v).x == ret(ret(v).x).x  ` ),
       ...
       h('p', ' If the values of Monad instances are updated only through the use of the Monad ret() method, then the current state of the Monad instances exists in the mutable, global object named "O". Keeping changing monad state in one place (on the object "O") makes applications easier to reason about and easier to maintain. I treat Monad instances as though they were immutable, updating them only through the use of their ret() methods.   ' ),
        ...
        h('p', ' From the definition of Monad, you can see that m.x.ret(v) and O.m.ret(v) both result in O.mx == v. Also from the definition of Monad, it is apparent that m1.bnd(m2.ret) results in m2.ret(m1.x) being executed. After that operation, O.m2.x == m1.x. '  ),
        h('span.red3', 'cube(3)' ),
        ...
        h('p', 'For any instance of MonadIter, say "m", "m.bnd(func)" causes m.p == func. A call to "m.release(...args) causes p(...args) to execute, possibly with arguments supplied by the caller. Here is the definition: ' ), 
        code.monadIt,
        h('p', ' As shown later on this page, MonadIter instances control the routing of incoming websockets messages and the flow of action in the simulated dice game. In the demonstrations below, they behave much like ES2015 itterators. I prefer them over ES2015 itterators. '  ), 
...
//************************************************************************** BEGIN Promises
        ...
        h('p', ' Composisition with Promises involves chains of ".then" statements. Using MonadItter, composition can be accomplished in the usual monadic manner, using Monad\'s bnd() and ret() methods. ' ),
