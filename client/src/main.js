  ...
  const calcStream$ = merge( runTestAction$, quadAction$, testWAction$, testZAction$, testQAction$, edit1Action$, edit2Action$, taskAction$, colorAction$, deleteAction$, newTaskAction$, chatClickAction$, gameClickAction$, todoClickAction$, captionClickAction$, mM$3Action$, mM$1Action$, backClickAction$, forwardClickAction$, fibPressAction$, groupPressAction$, rollClickAction$, messagePressAction$, loginPressAction$, messages$, numClickAction$, opClickAction$ );
    ...
    return {
      DOM: 
        ...
        h('hr' ),
        h('h1', 'The Monads'  ),
        h('p', ' There are three basic types of monads: Monad, MonadIter, and MonadStream. Instances of Monad have a method called "bnd" which takes a functions and possibly other values as arguments. I have not created a comprehensive library of functions for bnd(), but most of the functions used in this demonstration are in the NPM package "js-monads" and all of them are in the Github repository. I create functions to suit specific purposes and I assume that anyone who uses my monads, or their own variations on the theme, will likewise want to create functions to suit their specific purposes. I use browserify or webpack so I can use ES6 functions such as map, reduce, and filter as components of the functions I create. ' ),
        h('p', ' There is a basic pattern that I have found useful for computations and manipulating objects, including arrays. For what it is worth, I will demonstrate my pattern with some simple examples in the discussion that follows. ' ),
        h('h2', 'Monad' ),
        code.monad,
