  ...
  const calcStream$ = merge( forwardAction$, backAction$, dummyAction$, newFibAction$, fibKeyPressAction5$, primeKeyPressAction5$, fibPressAction$, runTestAction$, quadAction$, testWAction$, testZAction$, testQAction$, edit1Action$, edit2Action$, colorAction$, deleteAction$, newTaskAction$, chatClickAction$, gameClickAction$, todoClickAction$, captionClickAction$, groupPressAction$, rollClickAction$, messagePressAction$, loginPressAction$, messages$, numClickAction$, opClickAction$ );
    ...
    return {
      DOM: 
        ...
        h('h2', 'Monad' ),
        code.monad,
       h('p', ' Monad instances maintain state in the unique, mutable, global object named "O". Where there is changing state, it is not practical to avoid mutating something. My choices narrowed down to the window object or an attribute of window like O. O seemed like the better choice. It is a place to keep the most recent versions of named monads. Earlier versions of named monads can persist elsewhere (as in the case of the traversable game display), or be left for the gargage collector.   ' ),
        h('p', ' In the examples shown on this page, the initial values of instances of Monad remain unchaged. The ret() method places updated instances of the monad calling ret() on O. The instances on O are never mutated. For any instance of Monad named m with id "m" and value v (i.e., m.x == v is true), m.ret(v2) creates a new attribute of O with key "m" or, if O.m already exists, m.ret(v2) mutates O by replacing its m attribute with a new monad whose x attribute is "v2". Before the change, O.m.x == v; after the change, O.m.x == v2. Nothing else changes. The prior O.m is not mutated, so any O.m that is replaced will persist if there is a reference to it, or will be subject to garbage collection if there is not. ' ),
        h('p', ' There is a basic pattern that I have found useful for computations and manipulating objects, including arrays. The functions add() and cube() are examples of the pattern. They will be defined and then used in the examples below.  ' ),
        h('h3', 'Examples' ),
