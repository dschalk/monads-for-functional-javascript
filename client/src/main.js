  ...
  const calcStream$ = merge( messages2$, timeoutAction$, factorsAction$, forwardAction$, backAction$, dummyAction$, primeFib$, fibPressAction$, runTestAction$, quadAction$, testWAction$, testZAction$, testQAction$, edit1Action$, edit2Action$, colorAction$, deleteAction$, newTaskAction$, chatClickAction$, gameClickAction$, todoClickAction$, captionClickAction$, groupPressAction$, rollClickAction$, messagePressAction$, loginPressAction$, messages$, numClickAction$, opClickAction$, requestAction$ );
    ...
    return {
      DOM: 
        ...
        h('hr' ),
        h('h1', 'The Monads'  ),     
        h('h3', ' Theoretical Considerations ' ),
        ...
        h('p', '  In other words, m.bnd(f ...args) = m2 for all monads m1 and for some monad m2,(which can be m1) in M, and all functions f in M. Left and Right identity, defined as m.bnd(m.ret) = m and m.ret(m.x) = m, is true where equality id defined as m1 = m2 if and only if m1.x = m2.x and m1.id = m2.id. The identities follow immediately from the definitions of the ret() and bnd() methods. The associativity requirement, m.bnd(f).bnd(g) = m.bnd(v => f(v).bnd(g) for all monads m and functions f in M, is always satisfied. By the definition of bnd(), m.bnd(f) returns f(m.x) and m.bnd(v => passes m.x to f(v).bnd(g), so both sides of the equation are equivalent to f(m.x).bnd(g). ' ),
        h('p', ' But instances of Monad can do much more than that. The functions provided as arguments to bnd() can return any Javascript value, including functions, objects, etc. Sometimes, at the end of a chain of computations, it is desirable to return a Javascript value that is not an instance of Monad. ' ),
        h('h3', ' Practical Matters '  ),
        h('p', 'As you will see, I adhere to certain restrictions regarding what I do with Monad instances as this makes the overall application (this web page) more maintainable, predictable, and organized. If I were working on a team effort, I would want everyone to to refrain from straying outside of certain guidelines. For example, the value held by an instance of Monad (defined below) should be changed only by use of the ret() method. Let m be an instance of Monad. m.ret(7) results in O.m.x == 7. m.x = 7 mutates m. O.m.x = 7 mutates O.m. The value of m, rather than O.m, can be changed without mutation by the expression ret(7, "m"), which creates a new m with m.x == 7 and m.id == "m". But avoiding that, and using only the ret() method, keeps the current states of all instances of Monad in a single location: on the unique global and mutable object O. Here is the definition of Monad:  '  ),
        h('h2', ' Monad ' ),
        code.monad,
    ...
    ret(v).x == ret(ret(v).x).x  ` ),
       ...
       h('p', ' If the values of Monad instances are updated only through the use of the Monad ret() method, then the current state of the Monad instances exists in the mutable, global object named "O". Keeping changing monad state in one place (on the object "O") makes applications easier to reason about and easier to maintain. I treat Monad instances as though they were immutable, updating them only through the use of their ret() methods.   ' ),
        ...
        h('span.td2',  'The console displays 27. m.bnd(v => sends m.x down the line, possibly providing m.x to several functions.' ),
        h('br' ),  
        h('br' ),  
        h('span.red3', 'cube(3).bnd(m.ret)' ),
        h('span.td2', ' Result: O.m.x == 27 and O.m.id == "m". ' ),
        h('br' ),  
        h('br' ),  
        h('span.red3', 'cube(10).ret(cube(3).x)' ),
        h('span.td2', ' O.anonymous.x == 27. O.anonymous.x == 1000 was true only for an instant, during the computation. ' ),
        h('br' ),  
        h('br' ),  
        ...
        h('span.td2', ' leaves m unchanged, but now O.m.x == 64, and O.m.id == "m". ' ),
        h('br' ),  
        h('br' ),  
        h('span.tao', ' By the way, If you want to mutate m, ' ),
        h('span.red3', 'ret(newVal, "m")' ),
        h('span', ' will do the job. After that operation,  m.x == newVal and m.id = "m". I haven\'t found a need to do that sort of thing. I like to confine changing monad state to the mutable, global object "O", and leave the plain monads alone. That keeps the application tidy and manageable.  '  ),   
        ...
        h('span.td2', ' causes O.m.x == 7 and leaves O.m.id unchanged (still "m"). ' ),
        h('br' ),  
        h('br' ),  
        ...
        h('p', 'For any instance of MonadItter, say "m", "m.bnd(func)" causes m.p == func. A call to "m.release(...args) causes p(...args) to execute, possibly with arguments supplied by the caller. Here is the definition: ' ), 
        code.monadIt,
        h('p', ' As shown later on this page, MonadItter instances control the routing of incoming websockets messages and the flow of action in the simulated dice game. In the demonstrations below, they behave much like ES2015 iterators. I prefer them over ES2015 iterators. '  ), 
        ...
        code.testZ,
        h('p', ' add() and cube() are defined in the Monad section (above). If you click "mMZ1.release(1)" several times, the code (above) will run several times, each time with v == 1. The result, O.mMt3.x, is shown below the button. mMZ1.p (bnd()\'s argument) remains constant while mMZ1.release(1) is repeatedly called, incrementing the number being cubed each time. ' ),
...
//************************************************************************** BEGIN Promises
        ...
        h('p', 'The "anonymous monad" isn\'t entirely anonymous. True, it doesn\'t have a name, but O.anonymous holds the result of calling cube with only two arguments. "data" is O.anonymous in the expression "data => m.ret(data.x).bnd(add, 15, m)" ' ),
        h('p', ' The same result can be achieved with MonadItter instead of Promises. Consider this: ' ),
        code.timeout,
        h('p', ' The following code uses timeout2 (above). If you click RUN, "O.m.x is 27" appears after one second. Two seconds later, "O.m.x is 42" is displayed along with a blurb that confirms the chain can continue after the delayed computation completes. ' ),
        code.timeoutSnippet,
        h('p', ' '  ),
        h('button#timeout',  ' Run ' ),
        h('span#timeout2'  ), 
        h('span#timeout3'  ), 
        h('p', ' Here is a screen shot showing the result of running similar code in the Chrome console: ' ),
        h('img', {props: {src: 'timeout.png', style: "max-height: 1400px; max-width: 1400px;" }} ),
        ...
        h('p', ' I didn\'t provide for error handling. There doesn\'t seem to be any need for it in this demonstration. If I were getting information from a remote database or Ajax server, I would handle errors with "window.addEventListener("error", function (e) { ...".' ),
...
//************************************************************************** END Promises
       ...
       code.MonadSet,
       h('p', ' Because sMplayerss is immutable, its most recent state can be safely stored in the O.mMsetArchive instance of Monad. This is done so the traversable game history shows who was online in each step. Here is the code that keeps the browser window current and, at the same time, maintains a history of the sate of game play. ' ),
       code.traverse,
