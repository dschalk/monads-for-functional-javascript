...
function main(sources) {
 ...
 var updateScoreboard2 = function updateScoreboard(v) {
  ...
  const colorAction$ = colorClick$.map(e => {
    let ind = getIndex(e);
    let index = parseInt(ind,10)
    let s = mMcurrentList.x;
    let ar = s.split(',');
    let n: number = 6 * index + 3;
    let j = 6 * index + 2;
    let k = 6 * index + 1;
    let checked = ar[n];
    if (checked == 'true')  {
  ...
  const deleteAction$ = deleteClick$.map(e => {
    let index = parseInt(getIndex(e),10);
    let s = mMcurrentList.x;
    let ar = s.split(',');
    let str = '';
    ar.splice(index*6, 6)
...
//**************************************   GAME   *********************************************** GAME END
  ...
  var fib2 = function fib2 (v) {
      if (v[2] > 1) {mMfib.ret([v[1], v[0] + v[1], v[2] -1])}
      else {
        console.log(v[0]);
        mM19.ret(v[0]);
      }
...
// <>>><>><><><><>>>><><><  traversal  ><><><><><><>>><><><><><><><><><><><>< START Itterator  
  ...
  mMZ2.bnd(v => cube(v, mMtemp5).bnd(w => mMt3.ret(v + ' cubed is ' + w))) 
