	...
	function main(sources) {
	  ...
	  var timeoutAction$ = timeoutClicks$.map(function () {
	    ...
	    m.ret(3, 'm').bnd(timeout2, 1, m, [function () {
	      return O.m.bnd(cube, m).bnd(send, 'timeout2', 'O.m.x is ' + ' ' + O.m.x, O.m).bnd(timeout2, 2, m, [function () {
	        return O.m.bnd(add, 15, m).bnd(send, 'timeout2', 'O.m.x is ' + ' ' + O.m.x, O.m)
	        /* Continue chaining from here */
	        .bnd(send, 'timeout3', 'The meaning of everything was computed to be' + ' ' + O.m.x, O.m);
	      }]);
	    }]);
	  ...
	  var solve = function solve() {
	    mMZ3.bnd(function (a) {
	      return mMtemp.ret(a).bnd(innerHTML, '', 'quad6', mMtemp).bnd(innerHTML, a + " * x * x ", 'quad5', mMtemp).bnd(function (a) {
	        return mMZ3.bnd(function (b) {
	          return mMtemp.ret(b).bnd(innerHTML, " + " + b + " * x ", 'quad6', mMtemp).bnd(function (b) {
